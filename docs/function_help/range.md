...
- `range(start, stop)`: integer half-open range. The default step is `1` when `start <= stop` and `-1` when `start > stop`.
- `range(start, stop, step)`: numeric half-open range. `step` may be positive or negative, but must not be zero.

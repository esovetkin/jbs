// lowers table-valued globals to `parameterset` YAML entries
//
...
func (ctx *lowerContext) ensureSourceParameterSet(source string) bool {
	if source == "" {
		return false
	}
	if _, emitted := ctx.sourceParameterSetEmitted[source]; emitted {
		return true
	}
	binding := ctx.res.BindingsByName[source]
	if binding == nil || binding.Shape != sema.BindingTable {
		return false
	}
	ctx.doc.ParameterSet = append(ctx.doc.ParameterSet, lowerGlobalBinding(binding, ctx.diags))
	ctx.sourceParameterSetEmitted[source] = struct{}{}
	ctx.names[source] = struct{}{}
	return true
}

func lowerGlobalBinding(binding *sema.GlobalBinding, diags *diag.Diagnostics) ParameterSet {
	out := ParameterSet{
		Name:      binding.Name,
		Parameter: make([]Parameter, 0),
		Meta: ParameterSetMeta{
			Kind:   ParameterSetKindGlobalTable,
			Source: binding.Name,
		},
	}

	rowCount := len(binding.Rows)
	if rowCount == 0 {
		for _, name := range binding.Order {
			if n := len(binding.Vars[name]); n > rowCount {
				rowCount = n
			}
		}
	}
	if rowCount == 0 {
		diags.AddError(
			diag.CodeE230,
			fmt.Sprintf("table-valued global '%s' evaluates to zero rows", binding.Name),
			binding.Span,
			"ensure the global binding yields at least one row",
		)
		rowCount = 1
	}

	valuesByName := make(map[string][]eval.Value, len(binding.Order))
	for _, name := range binding.Order {
		valuesByName[name] = valuesFor(binding, name, rowCount)
	}

	indices := planutil.SequentialIndices(rowCount)
	out.Parameter = lowerIndexedParameters(binding.Order, valuesByName, binding.Modes, indices, indexVariableName(binding.Name), func(name string) diag.Span {
		return originFor(binding, name)
	}, diags)
	return out
}
...
func lowerIndexedPayloadParameters(
	...
	for _, name := range order {
		...
		if mode := modes[name]; mode != "" {
			parameter := Parameter{Name: name, Mode: mode}
			switch mode {
			case "python":
...
func originFor(binding *sema.GlobalBinding, name string) diag.Span {
	if s, ok := binding.Origins[name]; ok {
		return s
	}
	return binding.Span
}

func valuesFor(binding *sema.GlobalBinding, name string, rowCount int) []eval.Value {

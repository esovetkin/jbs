...
func warnUnusedGlobalLocalsTest(assigns map[string]localAssignMeta, order, seed []string, diags *diag.Diagnostics) {
	if len(assigns) == 0 || len(seed) == 0 {
		return
	}
	warnUnusedGlobalContributors(assigns, order, nil, nil, seed, diags)
}
...
func TestWarnUnusedGlobalLocals_EarlyReturnAndReachability(t *testing.T) {
	...
	warnUnusedGlobalLocalsTest(assigns, order, nil, diags)
	if len(diags.Items) != 0 {
		t.Fatalf("expected no diagnostics when seed is empty, got: %s", diags.String())
	}

	diags = &diag.Diagnostics{}
	warnUnusedGlobalLocalsTest(nil, order, []string{"a"}, diags)
	if len(diags.Items) != 0 {
		t.Fatalf("expected no diagnostics when assigns is empty, got: %s", diags.String())
	}

	diags = &diag.Diagnostics{}
	warnUnusedGlobalLocalsTest(assigns, order, []string{"missing", "a"}, diags)
...
func containsWarningForVar(diags *diag.Diagnostics, name string) bool {
	target := "global helper variable '" + name + "'"
...
func TestWarnUnusedGlobalContributors_ShadowedImportedStillWarns(t *testing.T) {
	sp := func(off int) diag.Span {
		return diag.NewSpan("in.jbs", diag.NewPos(off, 1, off+1), diag.NewPos(off+1, 1, off+2))
	}
	assigns := map[string]localAssignMeta{
	...
	diags := &diag.Diagnostics{}
	warnUnusedGlobalContributors(assigns, []string{"x"}, imported, []string{"x"}, []string{"x"}, diags)
...
func TestWarnUnusedGlobalContributors_SelfRebindMarksImportedUsed(t *testing.T) {
	sp := func(off int) diag.Span {
		return diag.NewSpan("in.jbs", diag.NewPos(off, 1, off+1), diag.NewPos(off+1, 1, off+2))
	}
	assigns := map[string]localAssignMeta{
...
func TestWarnUnusedGlobalContributors_AdditionalBranches(t *testing.T) {
	sp := func(off int) diag.Span {
		return diag.NewSpan("in.jbs", diag.NewPos(off, 1, off+1), diag.NewPos(off+1, 1, off+2))
	}
	...
	t.Run("both empty returns early", func(t *testing.T) {
		diags := &diag.Diagnostics{}
		warnUnusedGlobalContributors(nil, nil, nil, nil, []string{"x"}, diags)
		if len(diags.Items) != 0 {

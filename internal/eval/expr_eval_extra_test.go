...
func TestBuiltinCallNames(t *testing.T) {
	...
	for _, name := range []string{"bool", "env", "range", "rev", "table", "t", "map", "reduce", "rename", "sum", "prod", "head", "tail", "print", "read_csv", "shell"} {
	...
	for _, name := range []string{"range", "table", "t", "shell", "env", "rename", "sum", "prod", "head", "tail"} {

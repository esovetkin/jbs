...
func TestEvalShellCallAssumeStringDoesNotRun(t *testing.T) {
	calledEnviron := false
	uses := make([]ShellUseEvent, 0)
	diags := &diag.Diagnostics{}
	got := EvalExprWithOptions(shellCall(ast.StringExpr{Value: "printf $x"}), map[string]Value{
		"x": Int(7),
	}, diags, ExprOptions{
		...
		Environ: func() []string {
			calledEnviron = true
			return []string{"BASE=from-provider"}
		},
	})
	if diags.HasErrors() {
		t.Fatalf("unexpected diagnostics: %s", diags.String())
	...
	if got.Kind != KindString || got.S != "" {
		t.Fatalf("unexpected assumed shell result: %#v", got)
	}
	if len(uses) != 1 || uses[0].Name != "x" || !uses[0].Scalar {
		t.Fatalf("unexpected shell use events: %#v", uses)
	}
	if !calledEnviron {
		t.Fatalf("expected environment provider to be called")
	}
}

func TestEvalShellCallAssumeStringStillValidatesArguments(t *testing.T) {
	tests := []struct {
		name string
		expr ast.Expr
	}{
		{
			name: "non string command",
			expr: shellCall(ast.NumberExpr{Int: true, IntValue: 1, Span: spanAt(720, 1)}),
		},
		{
			name: "non bool strip",
			expr: ast.CallExpr{
				Callee: ast.IdentExpr{Name: "shell", Span: spanAt(721, 1)},
				Args: []ast.CallArg{
					ast.PosCallArg(ast.StringExpr{Value: "true", Span: spanAt(721, 7)}),
					{Name: "strip", Expr: ast.StringExpr{Value: "no", Span: spanAt(721, 20)}, Span: spanAt(721, 20)},
				},
				Span: spanAt(721, 1),
			},
		},
...
func TestEvalShellCallAssumeStringNonScalarWarning(t *testing.T) {
	uses := make([]ShellUseEvent, 0)
	diags := &diag.Diagnostics{}
	got := EvalExprWithOptions(shellCall(ast.StringExpr{Value: "printf $x"}), map[string]Value{
		"x": List([]Value{Int(1)}),

...
`fsub` copies a template file into each workpackage directory, preserves its regular permission bits, and applies regular-expression substitutions using the step's visible variables before `run.sh` starts.
